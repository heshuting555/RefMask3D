...
def voxelize(
    ...
    for sample in batch:
        ...
        object_id.append(sample[8])
        input_ids.append(sample[9])
        attention_mask.append(sample[10])
        unique_multiple.append(sample[11])
    ...
    if "labels" in input_dict:
        ...
        is_unique = int(unique_multiple[0])
        ...
        if len(list_labels[0].shape) == 1:
            ...
            if mode == "test":
                for i in range(len(input_dict["labels"])):
                    target.append(
                        {"point2segment": input_dict["labels"][i][:, 0], "is_unique": is_unique}
                    )
                    target_full.append(
                        {
                            "point2segment": torch.from_numpy(
                                original_labels[i][:, 0]
                            ).long(), "is_unique": is_unique
                        }
                    )
            else:
                target = get_instance_masks(
                    ...
                    instance_id_batch=object_id,
                )
                for i in range(len(target)):
                    target[i]["point2segment"] = input_dict["labels"][i][:, 2]
                    target[i]["is_unique"] = is_unique
                if "train" not in mode:
                    ...
                    for i in range(len(target_full)):
                        target_full[i]["is_unique"] = is_unique
...
def get_instance_masks(
    ...
    for batch_id in range(len(list_labels)):
        ...
        masks = []
        segment_masks = []
        if instance_id_batch == []:
            instance_ids = list_labels[batch_id][:, 1].unique()
        else:
            instance_ids = [instance_id_batch[batch_id]]
