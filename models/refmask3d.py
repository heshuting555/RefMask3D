...
class RefMask3D(nn.Module):
    def __init__(
        ...
        for _ in range(num_shared):
            ...
            tmp_cross_test = nn.ModuleList()
            tmp_fusion_layers = nn.ModuleList()
            ...
            self.cross_test.append(tmp_cross_test)
            self.fusion_layers.append(tmp_fusion_layers)

        self.decoder_norm = nn.LayerNorm(hidden_dim)
        self.text_encoder = BertModel.from_pretrained("bert-base-uncased")
        print(config.freeze_text_encoder, 'whether freeze')
    ...
    def forward(
        ...
        pcd_features, aux = self.backbone(x, lang_feat_sentence, lang_mask.unsqueeze(dim=-1))

        batch_size = len(x.decomposed_coordinates)

        lang_feat_sentence = self.resizer(lang_feat_sentence)
        lang_feat = lang_feat_all.pooler_output  # B, Nl, 768
        ...
        predictions_mask = []
        predictions_embed = []
        predictions_embed.append(queries)
        predictions_text = []
        lang_feat_mask = lang_feat_sentence * lang_mask.unsqueeze(-1)
        lang_feat_mean = torch.sum(lang_feat_mask, dim=1) / lang_mask.sum(dim=1, keepdim=True)
        predictions_text.append(lang_feat_mean)
        for decoder_counter in range(self.num_decoders):
            ...
            for i, hlevel in enumerate(self.hlevels):
                ...
                src_pcd, lang_feat_fusion = self.fusion_layers[decoder_counter][i](
                        v=src_pcd.transpose(0, 1),
                        l=lang_feat_sentence,
                ...
                output = self.cross_test[decoder_counter][i](
                        output,
                        lang_feat_fusion.transpose(0, 1),
                ...
                predictions_embed.append(queries)
                lang_feat_mask = lang_feat_fusion * lang_mask.unsqueeze(-1)
                lang_feat_mean = torch.sum(lang_feat_mask, dim=1) / lang_mask.sum(dim=1, keepdim=True)
                predictions_text.append(lang_feat_mean)
        if self.train_on_segments:
        ...
        return {
            ...
            "pred_embed": predictions_embed[-1],
            "pred_text": predictions_text[-1],
            "aux_outputs": self._set_aux_loss(
                predictions_class, predictions_mask, predictions_embed, predictions_text),
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_seg_masks, outputs_pred_embed, outputs_pred_text):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [
            {"pred_logits": a, "pred_masks": b, "pred_embed": c, "pred_text": d}
            for a, b, c, d in zip(outputs_class[:-1], outputs_seg_masks[:-1], outputs_pred_embed[:-1], outputs_pred_text[:-1])
        ]
