...
class SetCriterionContrastive(nn.Module):
    ...
    def loss_embed(self, outputs, targets, indices, num_masks, mask_type):
        batch_idx, src_idx = self._get_src_permutation_idx(indices)
        pooled_image = outputs["pred_embed"]
        pooled_text = outputs["pred_text"].unsqueeze(1)
        loss_embeds = []

        for batch_id, (pool_image, pool_text) in enumerate(zip(pooled_image, pooled_text)):
            normalized_text_emb = F.normalize(pool_text, p=2, dim=-1)
            normalized_img_emb = F.normalize(pool_image, p=2, dim=-1)
            logits = torch.mm(normalized_text_emb, normalized_img_emb.t()) / self.temperature
            labels = torch.zeros(logits.size(0), dtype=torch.int64)
            labels[0] = src_idx[batch_id]
            loss_embed = F.cross_entropy(logits, labels.to(logits.device))
            loss_embeds.append(loss_embed)

        losses = {"loss_embed": torch.mean(torch.stack(loss_embeds))}
        return losses
    ...
    def loss_masks(self, outputs, targets, indices, num_masks, mask_type):
        ...
        return {
            "loss_mask": torch.mean(torch.stack(loss_masks)) * 4.0,
            "loss_dice": torch.mean(torch.stack(loss_dices)) * 4.0,
        }
    ...
    def get_loss(self, loss, outputs, targets, indices, num_masks, mask_type):
        loss_map = {"labels": self.loss_labels, "masks": self.loss_masks, "embed": self.loss_embed}
        assert loss in loss_map, f"do you really want to compute {loss} loss?"
        return loss_map[loss](outputs, targets, indices, num_masks, mask_type)
...
def match_indices(indice):
    result = []
    for i in indice:
        result.append([1 if x == i else 0 for x in indice])
    return result
